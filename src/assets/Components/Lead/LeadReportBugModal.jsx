...
import { createBugAPI, getMyProjectsAPI, getUsersAPI } from "../../../../services/allAPI";

function LeadReportBugModal({ onClose, getBugs }) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const reqBody = {
                ...
                project_id: Number(bugData.project_id),
                priority: bugData.priority,
                assigned_to: bugData.assigned_to ? Number(bugData.assigned_to) : null
            }

            const response = await createBugAPI(reqBody, reqHeader)
            ...
            if (response.status === 200) {
                ...
                toast.error(response?.response?.data?.message || 'Failed to report bug')
            }
        } catch (err) {
...
export default LeadReportBugModal;