...
function LeadEditBugModal({ bug, onClose, getBugs }) {
    ...
    const getProjectDevelopers = async () => {
        try {
            ...
            const response = await getProjectByIdAPI(bug.project_id, reqHeader)
            if (response.status === 200) {
                const devs = response.data.project.members.filter((m) => m.role === 'Developer')
                setDevelopers(devs)
            }
        } catch (err) {
...
export default LeadEditBugModal;